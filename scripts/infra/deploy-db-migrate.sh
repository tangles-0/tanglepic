...
if [[ "$APP_STACK_EXISTS" == "false" ]]; then
  echo "App stack does not exist yet. Building/pushing image and creating app stack at desiredCount=0."
...
if [[ "$APP_STACK_EXISTS" == "false" ]]; then
  echo "Completing first app deploy after successful migration..."
  pnpm --dir infra/cdk exec cdk deploy \
...
echo "DB migration completed for env=${ENVIRONMENT}."
